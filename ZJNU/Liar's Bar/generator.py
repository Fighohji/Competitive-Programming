...
if __name__ == "__main__":
    ...
    for i in range(T):
        ...
        # tables 1 line
        print(''.join([random.choice(tables) for i in range(24)]))
        ...
        for j in range(24):
            ...
            for x in range(4):
                # pokers order 1 line
                poker = pokers[x * 5:x * 5 + 5]
                k = random.randint(2, 5)
                cut = random.sample(range(1, 5), k - 1)
                if k == 2:
                    cut = random.sample(range(2, 4), 1)
                cut = sorted(cut)
                cut = [0] + cut + [5]
                print(k, end=' ')
                res = []
                ...
                for p in range(1, k + 1):
                    res.append(''.join(poker[cut[p - 1]:cut[p]]))
                print(' '.join(res))

                # questions 1 line
                print(''.join('?' if random.randint(0, 100) % 101 == 0 else '.' for y in range(5)))
