...
struct Info {
    ll sum, pre, suf, Max;
    Info() { sum = 0, pre = -LINF, suf = -LINF, Max = -LINF; }
    Info(ll _pre, ll _suf, ll _sum, ll _Max) {
        pre = _pre;
        suf = _suf;
        sum = _sum;
        Max = _Max;
    ...
    Info operator+(const Info &t) const {
        ...
        i.suf = max(t.suf, suf + t.sum);
        i.Max = max({Max, t.Max, suf + t.pre});
        return i;
    }
};
...
void solve() {
    ...
    while (q--) {
        ...
        if (op == 1) {
            int x, y; cin >> x >> y;
            seg.upd(1, 1, n, x, Info(-y, y, y, -LINF));
        } else {
            int l, r; cin >> l >> r;
