...
void solve() {
    ...
    vector<int> in(n << 1), out(n << 1), dep(n << 1), siz(n + 1), euler;
    int idx = 0;
    ...
    SegTree seg(init);
    vector<vector<array<int, 3>>> del(n + 1);
    for (int i = 1; i < n; ++i) {
        auto &[u, v, w] = edge[i];
        if (dep[u] < dep[v]) swap(u, v);
        del[min(siz[u], n - siz[u])].push_back({u, v, w});
    }
    vector<vector<array<int, 3>>> qry(n + 1);
    for (int i = 1; i <= q; ++i) {
        int a, b, k;
        cin >> a >> b >> k;
        qry[k].push_back({a, b, i});
    }
    ...
    for (int i = 1; i <= n; ++i) {
        for (auto &[a, b, id] : qry[i]) {
            auto &[u, v, w] = edge[a];
            if (min(siz[u], n - siz[u]) < i) { w = 0, b = 0; }
            if (w != b) seg.upd(1, 0, idx - 1, in[u], out[u], b - w);
            Ans[id] = seg.node[1].d;
            if (w != b) seg.upd(1, 0, idx - 1, in[u], out[u], w - b);
        }
        for (const auto &[u, v, w] : del[i]) {
            seg.upd(1, 0, idx - 1, in[u], out[u], -w);
        }
    }
    for (int i = 1; i <= q; ++i) {
        cout << Ans[i] << '\n';
